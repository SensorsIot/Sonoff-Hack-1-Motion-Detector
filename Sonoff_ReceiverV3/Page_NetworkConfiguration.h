...
void send_network_configuration_html()
{
	...
	if (webServer.args() > 0 )  // Save Settings
	{
		...
		for ( uint8_t i = 0; i < webServer.args(); i++ ) {
			if (webServer.argName(i) == "ssid") ssid =   urldecode(webServer.arg(i));
			if (webServer.argName(i) == "password") password =    urldecode(webServer.arg(i));
			if (webServer.argName(i) == "ip_0") if (checkRange(webServer.arg(i))) 	config.IP[0] =  webServer.arg(i).toInt();
			if (webServer.argName(i) == "ip_1") if (checkRange(webServer.arg(i))) 	config.IP[1] =  webServer.arg(i).toInt();
			if (webServer.argName(i) == "ip_2") if (checkRange(webServer.arg(i))) 	config.IP[2] =  webServer.arg(i).toInt();
			if (webServer.argName(i) == "ip_3") if (checkRange(webServer.arg(i))) 	config.IP[3] =  webServer.arg(i).toInt();
			if (webServer.argName(i) == "nm_0") if (checkRange(webServer.arg(i))) 	config.Netmask[0] =  webServer.arg(i).toInt();
			if (webServer.argName(i) == "nm_1") if (checkRange(webServer.arg(i))) 	config.Netmask[1] =  webServer.arg(i).toInt();
			if (webServer.argName(i) == "nm_2") if (checkRange(webServer.arg(i))) 	config.Netmask[2] =  webServer.arg(i).toInt();
			if (webServer.argName(i) == "nm_3") if (checkRange(webServer.arg(i))) 	config.Netmask[3] =  webServer.arg(i).toInt();
			if (webServer.argName(i) == "gw_0") if (checkRange(webServer.arg(i))) 	config.Gateway[0] =  webServer.arg(i).toInt();
			if (webServer.argName(i) == "gw_1") if (checkRange(webServer.arg(i))) 	config.Gateway[1] =  webServer.arg(i).toInt();
			if (webServer.argName(i) == "gw_2") if (checkRange(webServer.arg(i))) 	config.Gateway[2] =  webServer.arg(i).toInt();
			if (webServer.argName(i) == "gw_3") if (checkRange(webServer.arg(i))) 	config.Gateway[3] =  webServer.arg(i).toInt();
			if (webServer.argName(i) == "dhcp") config.dhcp = true;
		}
		 webServer.send ( 200, "text/html", PAGE_WaitAndReload );
		writeConfig();
	}
	else
	{
		webServer.send ( 200, "text/html", PAGE_NetworkConfiguration ); 
	}
...
void send_network_configuration_values_html()
{
	...
	values += "ssid|" +           ssid + "|input\n";
	values += "password|" +       password + "|input\n";
